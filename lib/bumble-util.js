...
(function() {
  var HOME_DIR, LAST_NPM_INSTALL_FILE, _, child_process, fs, handleError, installNodePackage, moment, npmInstall, openTerminalTab, path, systemCmd;
  ...
  installNodePackage = function(packageName, options) {
    ...
    ref = options.global ? ['-g', 'sudo'] : ['', ''], flags = ref[0], sudo = ref[1];
    if (!(fs.existsSync("/usr/local/lib/node_modules/" + packageName) || fs.existsSync("/opt/nodejs/current/lib/node_modules/" + packageName))) {
      if (options.global) {
        console.log('you may be asked to enter your sudo password (and this may take a few seconds)');
      }
      return systemCmd(sudo + " npm install " + flags + " " + packageName);
    }
  };
